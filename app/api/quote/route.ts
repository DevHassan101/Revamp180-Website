...
export async function POST(request: Request) {
  ...
  if (!SMTP_HOST || !SMTP_USER || !SMTP_PASS) {
    console.error("Quote form: SMTP environment variables are not configured.");
    return NextResponse.json(
  ...
  const safe = {
    ...
    features: escapeHtml(features || "—").replace(/\n/g, "<br/>"),
  ...
  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;color:#1a1a2e">
      <div style="background:linear-gradient(135deg,#080B78,#00004D);padding:24px;border-radius:12px 12px 0 0">
        <h2 style="color:#fff;margin:0;font-size:18px">New quote request from revamp180.com</h2>
        <p style="color:#C0BAFF;margin:6px 0 0;font-size:13px">${safe.service}</p>
      </div>
      <div style="border:1px solid #e5e5f0;border-top:none;border-radius:0 0 12px 12px;padding:24px">
        ...
        ${row("Company / Business", safe.business)}
        ...
        <hr style="border:none;border-top:1px solid #e5e5f0;margin:16px 0"/>
        <p style="margin:0 0 6px"><strong>Project description</strong></p>
        <p style="margin:0 0 16px;line-height:1.6;color:#333">${safe.description}</p>
        <p style="margin:0 0 6px"><strong>Key features / requirements</strong></p>
        <p style="margin:0;line-height:1.6;color:#333">${safe.features}</p>
      </div>
    </div>
  `;

  const text =
    `New quote request from revamp180.com\n\n` +
    `Name: ${fullName}\n` +
    `Email: ${email}\n` +
    `Phone: ${phone}\n` +
  ...
  try {
    await transporter.sendMail({
      ...
      subject: `[Quote] ${service} — ${fullName}`,
      text,
      html,
    });
  } catch (err) {
    console.error("Quote form: failed to send email", err);
    return NextResponse.json(
